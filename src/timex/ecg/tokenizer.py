...
class ECGTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        self,
        SAX_List: Type[OneD_SymbolicAggregateApproximation]
        | Type[SymbolicAggregateApproximation]
        | PiecewiseAggregateApproximation
        | None = None,
        unk_token: str = "<unk>",
        bos_token: str = "<s>",
        ...
        beat_strategy: Literal[
            "channel", "product"
        ] = "product",  # 'channel' or 'product'
        ...
        **kwargs,
        ...
        beat_token = (
            AddedToken(beat_token, lstrip=True, rstrip=False)
        ...
        self.min_peak_distance = (
            (350 * self.sample_rate) / 500
        )  # Minimum distance between R-peaks in samples (for 500 Hz, 200 samples = 400 ms)
    ...
    def _convert_token_to_id(self, token: str) -> int:
        ...
        return (
            ...
            else self.tokenizer.token_to_id(self.unk_token.content)
        )
    ...
    def _decode(self, token_ids, skip_special_tokens: bool = True, **kwargs):
        ...
        for i in range(len(decoded)):
            sax_decoded.append(
                self.SAX_List.inverse_transform(
                    np.array(decoded[i]).reshape(-1, 1)
                ).reshape(-1)
            )
    ...
    def token_to_string(self, data):
        return [
            [[str(token) for token in segment] for segment in lead] for lead in data
        ]
    ...
    def pad_signal(self, data, target_length):
        ...
        for i in range(len(data)):
            for j in range(len(data[i])):
                ...
                sig = data[i][j]
                padded_signals[i][j, : len(sig)] = (
                    sig  # This copies the original signal in the beginning
                )
    ...
    def encode(
        ...
        for i in range(n_signals):
            ...
            for j in range(n_leads):
                ...
                lead_ids = [
                    self.vocab.get(str(tok), self.vocab["<unk>"]) for tok in lead_tokens
                ]
            ...
            all_encodings.append(
                {
                    ...
                    "attention_mask": torch.tensor(
                        flat_attention_mask, dtype=torch.long
                    ),
                }
            )
    ...
    def identify_beatmarks(self, array):
        ...
        if self.beat_strategy == "channel":
            simple_peaks = find_peaks(
                array[self.beat_channel_idx], distance=self.min_peak_distance
            )
        ...
        # Convert to segments
        self.beat_segments = [
            int(pidx // self.segment_length) for pidx in simple_peaks[0]
        ]
    ...
    def compute_beat_based_attention_mask(self, ecg_data):
        ...
        _, rpeaks = nk.ecg_peaks(
            ecg_data[self.beat_channel_idx], sampling_rate=self.fs_out
        )  # compute R peaks from II
        signal_dwt, _ = nk.ecg_delineate(
            ecg_data[self.beat_channel_idx],
            rpeaks,
            sampling_rate=500,
            method="dwt",
            show=False,
        ...
        p_wave = (
            signal_dwt["ECG_P_Onsets"] | signal_dwt["ECG_P_Offsets"]
        )  # binary serie with 1 where P waves start and stop
        qrs_complex = (
            signal_dwt["ECG_Q_Peaks"] | signal_dwt["ECG_S_Peaks"]
        )  # binary serie with 1 where QRS complexes start and stop
        t_wave = (
            signal_dwt["ECG_T_Onsets"] | signal_dwt["ECG_T_Offsets"]
        ...
        p_starts_stops = np.array(p_starts_stops).reshape(
            -1, 2
        ...
        t_starts_stops = np.array(t_starts_stops).reshape(
            -1, 2
        ...
        qrs_starts_stops = np.array(qrs_starts_stops).reshape(
            -1, 2
